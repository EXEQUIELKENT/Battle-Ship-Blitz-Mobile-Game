...
///    `vector-effect="non-scaling-stroke"`, meaning the ink stays the
///    same weight on screen no matter how small the ship is drawn. Since
///    the transform is applied to the PATH rather than the canvas, stroke
///    widths here are already in device pixels and simply don't scale —
///    which is exactly that rule. Without it a ship drawn at grid size
///    would be a solid blob of outline.
class FamilyCanvas {
  final Canvas canvas;
  ...
  FamilyCanvas._(this.canvas, this._m, this.scale);
  ...
  factory FamilyCanvas.stretch(Canvas canvas, Size size, Rect viewBox) {
    ...
    return FamilyCanvas._(canvas, m.storage, (sx + sy) / 2);
  }
  ...
  factory FamilyCanvas.fit(Canvas canvas, Size size, Rect viewBox) {
    ...
    final m = Matrix4.identity()
      ...
      ..translateByDouble(-viewBox.left, -viewBox.top, 0, 1);
    return FamilyCanvas._(canvas, m.storage, s);
  }
